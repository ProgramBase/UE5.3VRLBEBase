...
bool PXR_AnchorProvider::GetAnchorLoadResults(const FPICOSpatialHandle& FutureHandle, TArray<FAnchorLoadResult>& LoadResult, EPICOResult& Result)
{
	...
	if (QuerySenseDataComplete(FutureHandle, SenseDataQueryCompletion))
	{
		...
		{
			PXR_LOGE(PxrMR, "SnapShotHandle Invalid(),DestroySenseDataQueryResult Failed!");
		}
	}
	PXR_LOGV(PxrMR, "GetAnchorLoadResults Result:%d",bResult);
	return bResult;
}
...
bool PXR_SceneProvider::GetSpatialSceneInfos(const FPICOSpatialHandle& FutureHandle, TArray<FPICOMRSceneInfo>& SceneInfos, EPICOResult& Result)
{
	...
	PXR_LOGV(PxrMR, "GetSpatialSceneInfos Result:%d", bResult);

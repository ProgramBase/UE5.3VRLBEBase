...
bool UPICOXRMRFunctionLibrary::PXR_GetAnchorEntityUuid(AActor* BoundActor, FPICOSpatialUUID& OutAnchorUUID)
{
	...
	return PXR_AnchorProvider::GetInstance()->GetAnchorEntityUUID(BoundActor, OutAnchorUUID);
}
...
bool UPICOXRMRFunctionLibrary::PXR_GetAnchorPoseByActor(AActor* BoundActor, FTransform& OutTransform)
{
	...
	if (IsValid(AnchorComponent) && AnchorComponent->IsAnchorValid())
	{
		...
		return PXR_AnchorProvider::GetInstance()->GetAnchorPose(AnchorComponent, OutTransform);
	}

	return false;
}

bool UPICOXRMRFunctionLibrary::PXR_GetAnchorPoseByComponent(UPICOAnchorComponent* AnchorComponent, FTransform& OutTransform)
{
	if (IsValid(AnchorComponent) && AnchorComponent->IsAnchorValid())
	{
		if (FPICOProviderManager::ShouldUseLegacyMR())
...
bool UPICOXRMRFunctionLibrary::PXR_DestroyAnchorByComponent(UPICOAnchorComponent* AnchorComponent)
{
	bool bResult=false;
	...
	if (IsValid(AnchorComponent) && AnchorComponent->IsAnchorValid())
	{
		...
		{
			bResult=PXR_AnchorProvider::GetInstance()->DestroyAnchorByHandle(AnchorComponent->GetAnchorHandle());
		}
...
bool UPICOXRMRFunctionLibrary::PXR_DestroyAnchorByActor(AActor* BoundActor)
{
	bool bResult=false;
	if (!IsValid(BoundActor) || !IsValid(BoundActor->GetWorld()))
	{

...
public:
	...
	bool StartProvider(const FPICOPollFutureDelegate& StartSenseDataProviderDelegate,EPICOResult& Result);
	bool StartSenseDataProviderComplete(const FPICOSpatialHandle& FutureHandle, FPICOSenseDataProviderStartCompletion& Completion);
	bool QuerySenseDataComplete(const FPICOSpatialHandle& FutureHandle, FPICOSenseDataQueryCompletion& Completion);
	bool GetQueriedSenseData(const FPICOSpatialHandle& QueryResultHandle, FPICOQueriedSenseData& QueriedSenseData);
	bool DestroySenseDataQueryResult(const FPICOSpatialHandle& QueryResultHandle);
...
public:
	...
	bool CreateSpatialAnchorAsync(const FPICOPollFutureDelegate& Delegate,const FTransform& InAnchorEntityTransform,EPICOResult& Result);
	bool PersistSpatialAnchorAsync(AActor* BoundActor, EPICOPersistLocation PersistLocation, const FPICOPollFutureDelegate& Delegate,EPICOResult& Result);
	bool UnpersistSpatialAnchorAsync(AActor* BoundActor,const FPICOPollFutureDelegate& Delegate,EPICOResult& Result);
	bool ShareSpatialAnchorAsync(AActor* BoundActor,const FPICOPollFutureDelegate& Delegate,EPICOResult& Result);
	bool LoadAnchorEntityAsync(const FPICOAnchorLoadInfo& LoadInfo, const FPICOPollFutureDelegate& Delegate,EPICOResult& Result);
	bool DownloadSharedSpatialAnchorsAsync(const FPICOAnchorLoadInfo& LoadInfo,const FPICOPollFutureDelegate& Delegate,TSet<FPICOSpatialHandle>& HandleSet,EPICOResult& Result);
	
	bool CreateSpatialAnchorComplete(const FPICOSpatialHandle& FutureHandle, FPICOSpatialAnchorCreateCompletion& completion);
	bool PersistSpatialAnchorComplete(const FPICOSpatialHandle& FutureHandle, FPICOSpatialAnchorPersistCompletion& completion);
	bool UnpersistSpatialAnchorComplete(const FPICOSpatialHandle& FutureHandle, FPICOSpatialAnchorUnpersistCompletion& completion);
	bool ShareSpatialAnchorComplete(const FPICOSpatialHandle& FutureHandle, FPICOSpatialAnchorShareCompletion& completion);
	bool DownloadSharedSpatialAnchorsComplete(const FPICOSpatialHandle& FutureHandle, FPICOSharedSpatialAnchorDownloadCompletion& completion);

	bool GetAnchorLoadResults(const FPICOSpatialHandle& FutureHandle,TArray<FAnchorLoadResult>& LoadResult,EPICOResult& Result);

	bool DestroyAnchorByHandle(const FPICOSpatialHandle& AnchorHandle);
	bool GetAnchorEntityUUID(AActor* BoundActor, FPICOSpatialUUID& OutAnchorUUID);
...
public:
	...
	bool RequestSpatialTriangleMesh(const FPICOPollFutureDelegate& Delegate,EPICOResult& Result);
...
private:
	...
	void SetLastUUIDToMRMeshInfoMap(const TMap<FPICOSpatialUUID,FPICOSpatialMeshInfo>& UUIDToMRMeshInfoMap);

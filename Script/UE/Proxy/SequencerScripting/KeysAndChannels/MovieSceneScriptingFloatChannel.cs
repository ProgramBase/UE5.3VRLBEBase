...
{
	[PathName("/Script/SequencerScripting.MovieSceneScriptingFloatChannel")]
	public partial class UMovieSceneScriptingFloatChannel : UMovieSceneScriptingChannel, IStaticClass
	{
		public new static UClass StaticClass()
		{
			return StaticClassSingleton ??= UObjectImplementation.UObject_StaticClassImplementation("/Script/SequencerScripting.MovieSceneScriptingFloatChannel");
		}
		...
		public virtual void SetDefault(float InDefaultValue)
		{
			unsafe
			{
				var __InBuffer = stackalloc byte[4];

				*(float*)(__InBuffer) = InDefaultValue;
		...
		/// <returns>
		/// An array of UMovieSceneScriptingFloatKey's contained by this channel.
		...
		/// <returns>
		/// An array of float's contained by this channel.
		/// Returns baked keys in the specified range.
		/// </returns>
		public virtual TArray<float> EvaluateKeys(FSequencerScriptingRange Range, FFrameRate FrameRate)
		{
			unsafe
			{
		...
		/// </returns>
		public virtual UMovieSceneScriptingFloatKey AddKey(FFrameNumber InTime, float NewValue, float SubFrame = 0.000000f, ESequenceTimeUnit TimeUnit = ESequenceTimeUnit.DisplayRate, EMovieSceneKeyInterpolation InInterpolation = EMovieSceneKeyInterpolation.Auto)
		{
			unsafe
			{
				...
				*(float*)(__InBuffer + 12) = SubFrame;

				*(byte*)(__InBuffer + 16) = (byte)TimeUnit;

				*(byte*)(__InBuffer + 17) = (byte)InInterpolation;
				...
				return *(UMovieSceneScriptingFloatKey*)__ReturnBuffer;
			}
		}

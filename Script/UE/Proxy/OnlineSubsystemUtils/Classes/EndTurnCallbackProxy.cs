...
{
	[PathName("/Script/OnlineSubsystemUtils.EndTurnCallbackProxy")]
	public partial class UEndTurnCallbackProxy : UOnlineBlueprintCallProxyBase, IStaticClass
	{
		...
		{
			return StaticClassSingleton ??= UObjectImplementation.UObject_StaticClassImplementation("/Script/OnlineSubsystemUtils.EndTurnCallbackProxy");
		}

		private static UClass StaticClassSingleton { get; set; }

		public static UEndTurnCallbackProxy EndTurn(UObject WorldContextObject, APlayerController PlayerController, FString MatchID, TScriptInterface<ITurnBasedMatchInterface> TurnBasedMatchInterface)
		{
			unsafe
			{
				...
				*(nint*)(__InBuffer + 24) = TurnBasedMatchInterface?.GarbageCollectionHandle ?? nint.Zero;

				var __ReturnBuffer = stackalloc byte[8];

				FFunctionImplementation.FFunction_CompoundCall11Implementation(StaticClass().GarbageCollectionHandle, __EndTurn, __InBuffer, __ReturnBuffer);

				return *(UEndTurnCallbackProxy*)__ReturnBuffer;
			}
		}
		...
		private static uint __EndTurn = 0;
	}
}